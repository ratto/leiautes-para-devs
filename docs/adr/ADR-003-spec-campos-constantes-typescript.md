...
A spec de campos de cada seção do leiaute é definida como constantes TypeScript exportadas de arquivos dedicados em `src/layouts/cnab240/`. Cada arquivo representa uma seção do leiaute (ex: `headerArquivo.ts`, `segmentoA.ts`). Os campos são tipados por uma interface ou type compartilhado (ex: `CampoLeiaute`).
...
Spec armazenada em `src/layouts/cnab240/header-arquivo.json` e importada pelos componentes.
...
Spec definida como constantes TypeScript em `src/layouts/cnab240/*.ts`.
...
1. - [ ] Definir interface `CampoLeiaute` em `src/layouts/types.ts` com os atributos obrigatórios de cada campo
2. - [ ] Criar `src/layouts/cnab240/headerArquivo.ts` como arquivo de referência para o padrão
