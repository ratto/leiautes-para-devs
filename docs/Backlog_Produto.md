...
**Nota de refinamento — divergências encontradas entre a descrição anterior e o código real:** a survey técnica desta sessão constatou que `src/utils/validation.ts` (`regrasCampo`/`regraObrigatorio`) **não checa `getModoPlayground` hoje**, que **não existe `q-form`/`formRef`** em `Cnab240Page.vue`, e que `TrailerLoteCard.vue`/`TrailerArquivoCard.vue` renderizam cada campo com `readonly`/`disable` fixos no template, sem nenhum mecanismo de override reativo. As decisões abaixo assumem esse débito dentro do escopo desta US (única integração real de `getModoPlayground` hoje é `CpfCnpjInput.vue`, para desabilitar máscara).

**Posicionamento e componente:** um novo componente `ModoToggle.vue` é montado na mesma linha do `TipoArquivoToggle.vue` em `Cnab240Page.vue`, com `justify-between` — remessa/retorno à esquerda, modo à direita. O toggle usa `QBtnToggle` do Quasar com opções `[{ label: 'Seguro', value: 'safe' }, { label: 'Playground', value: 'playground' }]`, com CSS scoped seguindo os tokens do projeto: estado ativo com `--lpd-accent` (background e borda) e `--lpd-on-accent` (texto); estado inativo com `--lpd-surface-2` e `--lpd-text-muted`. Decisão de refinamento: `TipoArquivoToggle.vue` (padrão atual, `q-btn` + `role="radiogroup"` manual) **não é alterado** por esta US — a inconsistência estrutural entre os dois toggles fica registrada como débito técnico para uma US futura de unificação.
...
**Bypass de validação (assume débito de US07):** `regrasCampo` e `regraObrigatorio`, em `src/utils/validation.ts`, passam a consultar `useConfigStore().getModoPlayground` internamente e retornam `[]`/`true` (sem executar nenhuma regra) quando o Playground está ativo. A checagem fica centralizada nas duas funções — nenhum call site nos cards (`HeaderArquivoCard`, `LoteCard`, `SegmentoACard`) precisa de lógica condicional adicional.

**q-form e formRef (assume débito de US07):** esta US envolve o conteúdo editável de `Cnab240Page.vue` em `<q-form ref="formRef">` e expõe a ref via `defineExpose`, criando a infraestrutura que a descrição anterior atribuía incorretamente a US07. US17 (download) poderá consumir o mesmo `formRef.validate()` de forma independente quando for implementada.
...
**Fora de escopo:** mensagens de erro específicas por campo (US08); campos `readonly` do Header de Arquivo, Header de Lote e Segmento A (não entram na lógica de override — apenas os Trailers); refatoração de `TipoArquivoToggle.vue` para `QBtnToggle` (fica como débito técnico); disparo de `validate()` no botão de download (US17, que reaproveitará o `formRef` criado aqui).

**Dependências:** depende de US07 (Done — fornece `modoPlayground`, `getModoPlayground`, `setPlaygroundState`/`togglePlayground` em `useConfigStore`; **não** fornece `formRef`, ao contrário do que constava antes — esta US assume essa lacuna). US10 não bloqueia nenhuma US identificada no backlog atual; US23 (Done) já consome `getModoPlayground` via `CpfCnpjInput` independentemente do toggle de UI desta US. US17 consumirá o `formRef` criado por esta US.
