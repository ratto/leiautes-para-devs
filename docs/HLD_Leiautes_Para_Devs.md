...
| `Cnab240RemessaForm` | Formulário completo de remessa CNAB240; hierarquia de cards colapsáveis | `useCnab240Store`, spec `src/layouts/cnab240/`, `validation.ts`, `masks.ts` |
| `Cnab240RetornoForm` | Formulário completo de retorno CNAB240; estrutura análoga ao de remessa | `useCnab240Store`, spec `src/layouts/cnab240/`, `validation.ts`, `masks.ts` |
...
| `FilePreviewModal` | Serializa o estado da store em linhas de 240 chars, exibe o arquivo com highlight de erros, oferece cópia para clipboard e download em ISO-8859-1 com CRLF | `useCnab240Store`, spec `src/layouts/cnab240/` |
| `useConfigStore` | Estado global: `modo` (remessa/retorno) e `validationMode` (seguro/playground) | Pinia |
| `useCnab240Store` | Estado editável pelo usuário: `headerArquivo: HeaderArquivo`, `lotes: LotesArquivo`; getters para `trailerLote` e `trailerArquivo` | Pinia, tipos de `src/layouts/cnab240/` |
| `src/layouts/cnab240/` | Constantes TypeScript tipadas definindo posição, tamanho e tipo de cada campo por seção (ex: `headerArquivo.ts`, `segmentoA.ts`) | Nenhuma |
...
- `FilePreviewModal` serializa o estado da store sob demanda usando a spec de `src/layouts/cnab240/`
...
- `HeaderArquivo` — campos do cabeçalho do arquivo; tipo definido em `src/layouts/cnab240/headerArquivo.ts`
...
- **Mitigação:**
  - Manter spec de campos isolada por leiaute em `src/layouts/`
...
- ADR 003 — Spec de campos como constantes TypeScript tipadas em `src/layouts/`
...
Decisões pendentes
- Quais segmentos de detalhe do CNAB240 entram no MVP (A, B, J, J52, O...) — bloqueante para definição da spec em `src/layouts/cnab240/`
...
- Implementar constantes de spec para os segmentos do MVP em `src/layouts/cnab240/`
